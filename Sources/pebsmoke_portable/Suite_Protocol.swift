...
//
// Integration: a harness main.swift calls `ProtocolSuite.run(check:)` with
// the same `(name, cond, detail) -> Void` shape every other pebsmoke suite
// in this codebase already uses (see pebsmoke_deterministic/main.swift).
...
public enum ProtocolSuite {
    ...
    public static func run(check: (_ name: String, _ cond: Bool, _ detail: String) -> Void) {
        goldenBytes(check)
        frameCodec(check)
        decodeErrors(check)
